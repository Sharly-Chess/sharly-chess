...
class PlaceCardTemplate:
    ...
    def __init__(
        ...
        template_data = toml.load(toml_file)
        # -------------------- type
        field = 'type'
        ...
        file_str: str
        try:
            file_str = str(template_data[field])
        except KeyError:
            file_str = f'{toml_file.stem}.html'
            logger.debug(
                '[%s]: property [%s] not set, defaults to [%s].',
                toml_file.name,
                field,
...
class PlaceCardPrintDocument(PrintDocument):
    ...
    @property
    def template_context(self) -> dict[str, Any]:
        return self._get_option(PlaceCardPrintOption).place_card_type.template_context(
            self
        )
