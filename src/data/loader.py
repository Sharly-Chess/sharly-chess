...
class EventLoader:
    ...
    _corrupted_uniq_ids: ClassVar[set[str]] = set()
...
class ArchiveLoader:
    ...
    def get_sorted_archives() -> list[Archive]:
        return sorted(
            [
                Archive(file, file.stem, datetime.fromtimestamp(file.lstat().st_mtime))
    ...
    def get_archive(cls, archive_name: str) -> Archive | None:
        ...
        return Archive(
            arch_file,
            arch_file.stem,
            datetime.fromtimestamp(arch_file.lstat().st_mtime),
        )
