...
class PlayerAdminController(BaseEventAdminController):
    ...
    def _admin_event_players_render(
        ...
        per_plugin_columns: Iterable[ExtraAdminColumn] = (
        ...
        players_clubs: list[Club] = sorted(
            {player.club for player in web_context.admin_event.players_by_id.values()}
        )
    ...
    async def htmx_admin_update_event_players(
        ...
        try:
            player_updater: AbstractPlayerUpdater = (
                PlayerUpdaterManager.updaters_by_id()[player_updater_id]
            )
        except KeyError:
