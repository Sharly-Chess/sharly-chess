...
class LoginThrottle:
    _attempts_by_key: ClassVar[dict[str, _Attempts]] = {}
    ...
    @classmethod
    def record_failure(cls, keys: list[ThrottleKey]) -> None:
        now = datetime.now()
        with cls._lock:
            for throttle_key in keys:
    ...
    @classmethod
    def record_success(cls, keys: list[ThrottleKey]) -> None:
        with cls._lock:
            for throttle_key in keys:
                cls._attempts_by_key.pop(throttle_key.key, None)

    @classmethod
    def _forget_old(cls, now: datetime) -> None:
