...
from common import experimental_features_enabled, REQUEST_TIMEOUT, TMP_DIR
...
class WebLibInstaller(ToolInstaller, ABC):
    ...
    lib_dir: Path = Path(__file__).parents[1] / 'web' / 'static' / 'lib'
...
class BootstrapInstaller(WebLibInstaller, ABC):
    ...
    def check_file(self) -> Path:
        print_interactive_info(str(self.version_install_dir / self.lib_files[0]))
