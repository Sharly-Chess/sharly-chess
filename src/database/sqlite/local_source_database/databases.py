...
class LocalSourceDatabase(SQLiteDatabase, IdentifiableEntity, ABC):
    ...
    UPDATE_TIMEOUT = 10
    #: Some federations' sites refuse the default agent of the HTTP library.
    _HEADERS: ClassVar[dict[str, str]] = {
        'User-Agent': f'SharlyChess/{SHARLY_CHESS_VERSION}'
    }

    #: The federation whose events activate the database, None for a
    #: database activated another way.
    ...
    def _schema(self) -> str:
        """SQL script creating the schema of the database."""
        # Default implementation - subclasses should override this if they don't use external generation
        raise NotImplementedError(
            'Subclass must implement _schema if _use_external_generator returns False'
        )
    ...
    def is_manual(self) -> bool:
        """Whether the source file cannot be downloaded by the application
        (a login is needed): the user gets it and installs it by hand."""
        return False

    @property
    def source_file_help(self) -> str:
        """Where the user gets the source file of a manual database."""
        return ''

    @property
    def source_file_accept(self) -> str:
        """The extensions of the source file the user can install, for the
        file input."""
        return ''
    ...
    def _populate_from_source_file(
        ...
        Database matches the schema of `_schema`."""
        # Default implementation - subclasses should override this if they don't use external generation
        raise NotImplementedError(
    ...
    def default_is_active(self) -> bool:
        """Whether the database is active until the user says otherwise."""
        return False

    @property
    def is_forced_active(self) -> bool:
        """Whether the database is active whatever the user says (a plugin
        needs it)."""
        return False

    @property
    def is_active_by_choice(self) -> bool:
        """Whether the database is active by the user's or a federation's
        choice, or by default: what gets it installed unasked, unlike a
        plugin's need for it."""
        stored_is_active = self.stored_source_database.is_active
    ...
    def is_active(self) -> bool:
        return self.is_forced_active or self.is_active_by_choice

    def _set_is_active(self, is_active: bool) -> None:
        stored_source_database = self.stored_source_database
        stored_source_database.is_active = is_active
        self.update_stored_source_database(stored_source_database)
    ...
    def _report_progress(self, message: str) -> None:
        """Tells what the update is doing, once a second at most on the
        screens."""
        cls = self.__class__
        cls.progress = message
        now = time()
        if now - cls._progress_published_at < 1:
            return
        cls._progress_published_at = now
    ...
    def content_description(self) -> str:
        """What the installed database holds, e.g. its number of players."""
        return ''

    def _count_rows(self, table: str) -> int | None:
        """The rows of a table of the installed database, remembered until
        the file changes."""
        cls = self.__class__
        if not self.exists():
            return None
        modified_at = self.file.stat().st_mtime
        if cls._content_count is None or cls._content_count[0] != modified_at:
            try:
                with self:
                    self.execute(f'SELECT COUNT(*) AS `count` FROM `{table}`')
                    count = int(self.fetchone()['count'])
            except DatabaseError:
                return None
            cls._content_count = (modified_at, count)
        return cls._content_count[1]
    ...
    def delete(self) -> None:
        super().delete()
        stored_source_database = self.default_stored_database
        stored_source_database.is_active = self.stored_source_database.is_active
        self.update_stored_source_database(stored_source_database)
        self.publish_database_status_updated()
    ...
    def _download_file(self, url: str, target: Path) -> bool:
        ...
        try:
            ...
            with open(target, 'wb') as f:
                for chunk in response.iter_content(chunk_size=1024 * 1024):
                    ...
                    self._report_progress(
                        _('Downloading… {received} / {total} MB').format(
                            received=received // 1_048_576, total=total // 1_048_576
                        )
                        if total
                        else _('Downloading… {received} MB').format(
            ...
            return False
        logger.info(
            self.log_prefix + 'Download complete (%.1f MB in %.1f s).',
            received / 1_048_576,
            time() - start,
        )
        return True
    ...
    def _fetch_page(self, url: str) -> str | None:
        """The text of a page of the federation's site, to find the link of
        a list published under a dated name; None on failure."""
        logger.info(self.log_prefix + 'Reading [%s]...', url)
        try:
            response: Response = get(
                url, headers=self._HEADERS, allow_redirects=True, timeout=60
            )
        except (ConnectionError, OSError) as ex:
            logger.exception(self.log_prefix + 'Could not read [%s]: %s.', url, ex)
            return None
        if response.status_code != 200:
            logger.error(
                self.log_prefix + 'Could not read [%s], error code [%d].',
                url,
                response.status_code,
            )
            return None
    ...
    def _download_and_unzip(self, url: str, source_file_dir: Path) -> bool:
        """Downloads the archive at *url* and extracts it in
        *source_file_dir*, returns False on failure."""
        zip_target = source_file_dir / 'download.zip'
        if not self._download_file(url, zip_target):
            return False
        try:
            with zipfile.ZipFile(zip_target, 'r') as zf:
                zf.extractall(source_file_dir)
        except (zipfile.BadZipFile, OSError) as ex:
            logger.exception(self.log_prefix + 'Could not extract [%s]: %s.', url, ex)
            return False
        finally:
    ...
    def _update(self, source_file: Path | None = None) -> None:
        """Update the source database:
        1. Download the source file (or take the one the user provides)
        ...
        with tempfile.TemporaryDirectory(dir=TEMPFILE_DIR) as tmpdir:
            ...
            if source_file is not None:
                logger.info(self.log_prefix + 'Installing [%s]…', source_file.name)
                try:
                    shutil.move(source_file, tmp_dir / self._source_file_name)
                    with suppress(OSError):
                        source_file.parent.rmdir()
                except OSError as e:
                    logger.exception(
                        self.log_prefix + 'Could not read [%s]: %s.', source_file, e
                    )
                    return self.stop_update(False)
            else:
            ...
            self._report_progress(_('Storing…'))
            ...
            try:
                ...
                self._report_progress(_('Indexing…'))
                with SQLiteDatabase(tmp_file, True) as database:
...
class GitHubLocalSourceDatabase(LocalSourceDatabase, ABC):
    ...
    def _download_from_github(
        ...
        return self._download_file(url, source_file_dir / remote_filename)
    ...
    def _download_zip_source_file(self, source_file_dir: Path) -> bool:
        ...
        self._report_progress(_('Extracting…'))
        try:
            with zipfile.ZipFile(zip_target, 'r') as zf:
    ...
    def _download_enc_source_file(self, source_file_dir: Path) -> bool:
        ...
        self._report_progress(_('Decrypting…'))
        try:
            AesEcb.decrypt_file(
                enc_target,
    ...
    def _generate_from_source_file(
        ...
        self._report_progress(_('Installing…'))
...
class LocalSourcePlayerDatabase(LocalSourceDatabase, ABC):
    ...
    @property
    def content_description(self) -> str:
        count = self._count_rows('player')
        if count is None:
            return ''
        return _('{count} players').format(count=Utils.localized_number(count))

    def _report_players_stored(self, count: int) -> None:
        self._report_progress(
            _('{count} players stored…').format(count=Utils.localized_number(count))
        )
...
class GitHubLocalSourcePlayerDatabase(
    GitHubLocalSourceDatabase, LocalSourcePlayerDatabase, ABC
):
    """A player database generated on GitHub."""
