...
from plugins.fra.fra_schools import PLUGIN_NAME, PLUGIN_DIR
...
class FRASchoolsDatabase(LocalSourceDatabase):
    ...
    def _download_source_file(self) -> bool:
        ...
        url: str = base_url + '?' + urllib.parse.urlencode({
            'select': ','.join([
                'code_departement',
                'libelle_departement',
                'nom_commune',
        ...
        if not json_file.exists():
            logger.error(
                self.log_prefix + 'No data received from [%s].', url
            )
            return False
        return True
    ...
    def normalize_name(name: str) -> str:
        ...
        name = re.sub(r"\b(De|Du|Des|La|Le|Les|Au|Aux|Et|En|Sur)\b",
    ...
    def _populate_from_source_file(self, database: SQLiteDatabase) -> bool:
        ...
        school_columns = [
            'school_id', 'school_name', 'department', 'commune', 'type', 'private'
        ]
        school_query = (
