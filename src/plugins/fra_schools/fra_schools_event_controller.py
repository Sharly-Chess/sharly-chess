...
from web.session import SessionPlayersSort, ListSessionVariable

get_data = partial(PluginUtils.get_plugin_data, PLUGIN_NAME)


class SessionPlayersFilterFRASchools(ListSessionVariable[int]):
    @property
    def key(self) -> str:
        return 'fra_schools_players_filter_schools'
...
class FraSchoolsAdminEventController(BaseEventAdminController):
    ...
    async def htmx_fra_schools_event_tab(
        ...
        if fra_schools_filter is not None:
            SessionPlayersFilterFRASchools(request).set(
