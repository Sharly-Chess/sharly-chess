...
from common.tool_installer import WebLibInstaller, ExecutableInstaller
...
def generate_license_files():
    ...
    ignored_packages = [
        'pip-licenses', 'prettytable', 'tomli', 'wcwidth',
    ...
    try:
                ...
                if 'LicenseFile' in package and package['LicenseFile']:
                    ...
                    try:
                        ...
                        if license_path.exists() and license_path.is_file():
                            with open(license_path, 'r', encoding='utf-8', errors='replace') as license_file:
                            ...
                            f.write(f"License file path provided but file not found: {license_file_path}\n")
                            f.write("Please refer to the package's PyPI page or repository for license details.\n")
                    except Exception as e:
                        logger.warning(f"Failed to read license file for {package_name}: {e}")
        ...
        logger.info(f'Created {len(individual_files_created)} individual package license files using pip-licenses')
    ...
    def process_installer(installer, install_dir, installer_type):
        ...
        if has_licence_files or has_licence_type:
            logger.debug(f'Scanning {installer_type.lower()}: {installer.name} in {install_dir}')

            # Create one combined file for this tool/library
            safe_tool_name = installer.name.replace('/', '_').replace(' ', '_').replace('-', '_')
            ...
            try:
                with open(individual_file, 'w', encoding='utf-8') as f:
                    ...
                    if has_licence_files:
                        sources.append(f"Source Licence Files: {', '.join(installer.licence_files)}")
                    if has_licence_type:
                        ...
                        for i, licence_file_path in enumerate(installer.licence_files):
                            ...
                            if licence_file.exists() and licence_file.is_file():
                                if len(installer.licence_files) > 1:
                                    f.write(f"LICENCE FILE {i+1}: {licence_file_path}\n")
                                    f.write("-" * 40 + "\n")
                                else:
                                ...
                                try:
                                    with open(licence_file, 'r', encoding='utf-8', errors='replace') as licence_content_file:
                                    ...
                                    logger.warning(f"Failed to read licence content from {licence_file}: {e}")
                                    f.write(f"Could not read licence file content: {e}\n")
                                    f.write(f"Licence file location: {licence_file}\n\n")

                                licence_info.append({
                                    'tool_name': installer.name,
                                    'tool_version': str(installer.version),
                                    'licence_file': licence_file,
                                    'individual_file': individual_file if individual_file.exists() else None
                                })
                            else:
                                logger.warning(f'Expected licence file not found: {licence_file} for {installer.name}')
                    ...
                    if has_licence_type:
                        ...
                        templates_dir = BASE_DIR / 'src' / 'common' / 'licence_templates'
                        template_file = templates_dir / f'{installer.licence_type}.txt'

                        try:
                            if template_file.exists():
                                with open(template_file, 'r', encoding='utf-8') as template_content_file:
                                    template_content = template_content_file.read()
                                f.write(template_content)
                                if not template_content.endswith('\n'):
                                    f.write('\n')
                            else:
                                f.write(f"Licence template not found for type: {installer.licence_type}\n")
                                f.write(f"Please add a template file at: {template_file}\n")
                                f.write("Or refer to the project's repository for licence details.\n")
                        except Exception as e:
                            logger.warning(f"Failed to read licence template {template_file}: {e}")
                        ...
                        licence_info.append({
                            'tool_name': installer.name,
                            'tool_version': str(installer.version),
                            'licence_file': template_file if template_file.exists() else None,
                ...
                logger.warning(f"Failed to create licence file for {installer.name}: {e}")

    # Process WebLibArchiveInstaller instances
    for installer in InstallationChecker.web_lib_installers:
        process_installer(installer, installer.version_install_dir, 'WebLibArchiveInstaller')
    ...
    logger.info(f'Collected {len(licence_info)} licence files from external tools and libraries')
    logger.info(f'Created {len(external_files_created)} individual external tool licence files')
