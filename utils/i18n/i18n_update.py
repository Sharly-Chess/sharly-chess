...
class LocaleInfo:
    ...
    def compare_message_tokens(self, msg: Message) -> bool:
        ...
        if isinstance(msg.id, str):
            if self.sorted_tokens(msg.id) != self.sorted_tokens(msg.string):
                msg.user_comments.append(f'Error: tokens differ between [{msg.id}] and [{msg.string}]')
                msg.string = ''
                error = True
        else:
            for i in reversed(range(len(msg.id))):
                if self.sorted_tokens(msg.id[i]) != self.sorted_tokens(msg.string[i]):
                    msg.user_comments.append(f'Error: tokens differ between [{msg.id[i]}] and [{msg.string[i]}]')
        ...
        for msg in catalog:
            if msg.id:
                ...
                for flag in msg.flags:
                    if flag not in ['python-format', 'error', ]:
    ...
    def write_markdown(self):
        ...
        with open(self.doc_file, 'w', encoding='utf-8') as f:
            ...
            f.write('## {text} ({num})\n\n'.format(
                text=_('Empty messages'), num=len(self.empty_optional_messages) or '-'))
            if self.empty_optional_messages:
                if self.default:
                    ...
                    f.write('|{text1}|{text2}|\n'.format(text1=_('Message ID'), text2=_('Locations')))
                    f.write('|--|--|\n')
                    for msg in self.empty_optional_messages.values():
                        if isinstance(msg.id, str):
                            text1: str = self.escape_gh_md(msg.id)
                        else:
                            text1: str = '**{s}** {st}<br/>**{p}** {pt}'.format(
    ...
    def print_summary(self):
        ...
        if self.empty_optional_messages:
            if self.id == default_locale:
                print_interactive_info(f'  * Empty optional messages ({len(self.empty_optional_messages)}), not listed for the default locale.')
            else:
                print_interactive_warning(f'  * Empty messages ({len(self.empty_optional_messages)})')
                for msg_id in list(self.empty_optional_messages.keys())[:empty_messages_max]:
...
class I18nUpdater:
    ...
    def __init__(
        ...
        for locale_info in self.locale_infos.values():
            locale_info.control()
            if locale_info.id not in trusted_locales and locale_info.empty_optional_messages:
