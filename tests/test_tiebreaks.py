...
class SwissTieBreaks(unittest.TestCase):
    ...
    def test_win(self):
        ...
        results = {
            player.id: tie_break_.compute_player_value_after(player, round_=None)
            for player in self.tournament.players_by_id.values()
        }
        expected = {
            2: 3,
    ...
    def test_buchholz_cut1(self):
        ...
        results = {
            i: tie_break_.compute_player_value_after(
                self.tournament.players_by_id[i], round_=None
            )
    ...
    def test_aob(self):
        aob = tie_break.AverageOfBuchholzTieBreak().compute_player_value_after
        results = {
            player.id: round(aob(player, round_=None), 2)
            for player in self.tournament.players_by_id.values()
        }
        expected = {
            2: 13.6,
    ...
    def test_ptp(self):
        ...
        self.assertEqual(
            tie_break_.compute_player_value_after(
                self.tournament.players_by_id[2], round_=None
            ),
            2217 # NOTE(Amaras): this should be 2216
        )
        self.assertEqual(
            tie_break_.compute_player_value_after(
                self.tournament.players_by_id[14], round_=None
            ),
    ...
    def test_average_perfect_performance_opponents(self):
        ...
        self.assertEqual(
            tie_break_.compute_player_value_after(
                self.tournament.players_by_id[3], round_=None
            ),
            1935 # NOTE(Amaras): should be 1934
        )
        self.assertEqual(
            tie_break_.compute_player_value_after(
                self.tournament.players_by_id[13], round_=None
            ),
