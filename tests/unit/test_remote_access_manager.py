...
from web.remote_access_session import RemoteAccessSession
...
def test_turning_it_on_again_after_a_lost_lease_opens_a_new_session(monkeypatch, ready):
    first = FakeSession()
    ...
    assert start_serving() is cast(RemoteAccessSession, second)
    assert len(opened) == 1
...
def test_taking_a_new_address_forgets_the_old_identity(monkeypatch, ready):
    ...
    retired: list[str] = []
    monkeypatch.setattr(remote_access_manager, 'retire_remote_access', retired.append)
    opening(monkeypatch, [])
    start_serving()
    old = server_identity()
