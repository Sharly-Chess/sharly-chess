...
def test_molter_recipe_resource_declares_supported_range() -> None:
    assert supported_molter_recipe_team_counts() == tuple(range(3, 26))
    assert MolterPairingSystem().supported_team_counts() == tuple(range(3, 26))
    assert available_molter_recipe_rounds(15, 12) == tuple(range(1, 15))
    assert available_molter_recipe_rounds(20, 12) == tuple(range(1, 20))
    assert available_molter_recipe_rounds(25, 12) == tuple(range(1, 25))
    assert available_molter_recipe_rounds(26, 12) == ()
...
def test_molter_recipe_table_reports_max_rounds_for_known_shape() -> None:
    table = get_molter_recipe_table(20, 12, 20)

    assert table is not None
    assert len(table.rounds) == 19


def test_molter_recipe_table_returns_none_for_missing_shape() -> None:
    assert get_molter_recipe_table(26, 12, 1) is None
