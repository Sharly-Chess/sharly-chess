...
from common import EVENTS_DIR, SHARLY_CHESS_VERSION
...
def remove_the_events_afterwards():
    ...
    for file in EVENTS_DIR.glob(f'test-snapshots-*.{Extension.EVENT_DB}'):
        if file.stem.endswith('-e2e'):
            continue
...
class TestRestore:
    ...
    def test_the_restoring_thread_keeps_its_own_access(self, event):
        ...
        with _restoring_event(event), EventDatabase(event, write=True) as database:
