...
@pytest.fixture
def relay(install_key: paramiko.PKey) -> Generator[Relay]:
...
def test_a_tunnel_that_drops_reads_as_coming_back_rather_than_as_stopped(
    ...
    reconnecting: TunnelStatus = tunnel.status
    assert reconnecting == TunnelStatus.RECONNECTING

    tunnel.stop()

    stopped: TunnelStatus = tunnel.status
    assert stopped == TunnelStatus.STOPPED
