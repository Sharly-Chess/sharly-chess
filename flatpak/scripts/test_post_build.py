...
class FlatpakPostBuildTester:
    ...
    def test_required_fields(self) -> bool:
        ...
        if missing:
            self.print_test(
                'Required fields present', False, f'Missing: {", ".join(missing)}'
            )
            return False
    ...
    def test_point_1_tcp_binding(self) -> bool:
        ...
        details = (
            ...
            if has_network
            else 'Missing --share=network'
        )
        self.print_test('Point 1: TCP Network Binding', has_network, details)
        return has_network
    ...
    def test_point_2_file_storage(self) -> bool:
        ...
        details = (
            'Can create & modify files in ~/.local/share/'
            if has_rw
            else 'Missing --filesystem=home:rw'
        )
        self.print_test('Point 2: File Storage (R/W)', has_rw, details)
        return has_rw
    ...
    def test_point_3_internet_deps(self) -> bool:
        ...
        details = (
            'Can download pip packages & external sources'
            if valid
            else 'Missing internet permission or sources'
        )

        self.print_test('Point 3: Internet Dependencies', valid, details)
        return valid
    ...
    def test_flatpak_info_available(self) -> bool:
        ...
        try:
            ...
            self.print_test('Flatpak tools available', available, version)
            return available
        except FileNotFoundError:
            self.print_test(
                'Flatpak tools available', False, 'flatpak command not found'
            )
            return False
