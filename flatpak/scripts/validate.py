...
class FlatpakManifestValidator:
    ...
    def validate_permissions(self) -> bool:
        ...
        found_display = any(arg in finish_args for arg in ['--socket=wayland', '--socket=x11'])
        if not found_display:
            self.warnings.append('No display socket specified (--socket=wayland or --socket=x11)')
...
def validate_requirements_file(req_path: Path) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if not issues:
            logger.info(f'Requirements file valid ({len([l for l in lines if l.strip() and not l.strip().startswith("#")])} packages)')
