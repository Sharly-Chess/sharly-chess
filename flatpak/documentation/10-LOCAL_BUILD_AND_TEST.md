...
```bash
# 1. Build the Flatpak (and install it in user mode)
...
flatpak-builder --verbose --force-clean --user --install build-dir flatpak/configuration/com.sharlychess.SharlyChess.json 2>&1 | tee build.log
```
...
flatpak-builder --force-clean build-dir flatpak/configuration/com.sharlychess.SharlyChess.json
...
```bash
# See: .github/workflows/publish-multiarch.yml

# Manual trigger
gh workflow run publish-multiarch.yml
```
...
flatpak-builder --verbose --force-clean --user --install build-dir flatpak/configuration/com.sharlychess.SharlyChess.json
