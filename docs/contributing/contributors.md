# _Sharly Chess_ - Maintainers, contributors and translators

## Maintainers

_Sharly Chess_ maintainers are the people in charge of the development and distribution of the application.

- Pascal AUBRY ([pascalaubry](https://github.com/pascalaubry))
- Sammy PLAT ([Amaras](https://github.com/Amaras))
- Timothy ARMES ([timothyarmes](https://github.com/timothyarmes))
- Youri AUBRY ([molrn](https://github.com/molrn))

## Contributors

_Sharly Chess_ contributors are people who have contributed some of the code or documentation.

- Pascal AUBRY ([pascalaubry](https://github.com/pascalaubry))
- Sammy PLAT ([Amaras](https://github.com/Amaras))
- Timothy ARMES ([timothyarmes](https://github.com/timothyarmes))
- Youri AUBRY ([molrn](https://github.com/molrn))
- Adrien AVENIA ([adrienave](https://github.com/adrienave))
- Arunprabhu KANDASAMY ([arun1587](https://github.com/arun1587))

See also:
- [Contributing guide](contributing-guide.md)

Since version 2.0, the source code is open for contributions on _GitHub_, it's up to you to be on the list of contributors!

## Translators

<!-- DO NOT EDIT! (START TRANSLATORS) -->

| Locale | Translators |
|--|--|
| <img src="../../src/web/static/images/locales/en.svg" style="height: 1em;"/>&nbsp;`en`&nbsp;English | [Timothy ARMES](https://github.com/timothyarmes) |
| <img src="../../src/web/static/images/locales/fr.svg" style="height: 1em;"/>&nbsp;`fr`&nbsp;Français | [Pascal AUBRY](https://github.com/pascalaubry), [Sammy PLAT](https://github.com/Amaras) |
| <img src="../../src/web/static/images/locales/pt.svg" style="height: 1em;"/>&nbsp;`pt`&nbsp;Português | [Emerson PROENÇA](https://github.com/emerson-proenca) |

<!-- DO NOT EDIT! (END TRANSLATORS) -->

See also:
- [Translation guide](translation-guide.md)

## Thanks

Thanks to Dominique RUHLMANN for adapting the _ChessEvent_ platform to allow the import from ChessEvent into _Sharly Chess_ and to all the beta testers!

See also:
- [Copyright](https://sharly-chess.com/copyright)
- [License AGPL](https://sharly-chess.com/license)
