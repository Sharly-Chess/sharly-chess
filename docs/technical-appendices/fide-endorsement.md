...
| ``--acceleration``, ``--no-acceleration`` | use the Baku acceleration method, or not |
| ``--tie-breaks`` | the criteria the standings are ranked on, as TRF26 acronyms; left out, the points and one to three of the mandatory tie-breaks |
...
paired by the pairing engine, the results are drawn from the ratings, and the
standings come from the tie-break engine. The file therefore states a tie-break
list and standings that follow from it (question 31), and nothing in it is
invented after the fact.

How often a game is drawn is taken from the model fitted in Otto Milvang's
*Statistical model for chess tournament simulations* (2024); how the decisive
games divide is set so that the expected score of each side is the one the
rating table gives for the difference between them (B.02 Art. 8.1). Question 32
asks that the rating variation of each player over a large sample be close to
zero, and the table is what a rating variation is measured against, so it
settles the expected score and the fitted model supplies only what the table
does not state. Neither colour is favoured: a colour advantage is a point the
rating table does not expect, and it cancels only for a player with as many
whites as blacks, which nobody has over an odd number of rounds.
