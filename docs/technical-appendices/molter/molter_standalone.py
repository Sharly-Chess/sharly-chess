...
Ideals, such as uniform team encounters, prefix opponent spread, and floater
balance, are reached only when arithmetic permits them. Reduced tables report
unmet ideals as notes, never as errors.

The construction is deterministic and portable. Full layers cover K_N in every
round, so I1 is exact. Partial layers reuse the same factors, advance them in a
prefix-balanced order, and assign them to board slots so every prefix meets as
many distinct opposing teams as possible. Every traversal is sorted and
deterministic, so a given (N, P, rounds) defines one reference table that can be
reproduced exactly in another language.
